...
list_of_shoes = input()
list_of_shoes = list(map(int, list_of_shoes.split()))
number_of_costumers = int(input())
money_earned = 0
for i in range(number_of_costumers):
    costumer = list(map(int, input().split()))
    shoe_number = costumer[0]
    money_purchased = costumer[1]
    for j in range(len(list_of_shoes)):
        if shoe_number == list_of_shoes[j]:
            list_of_shoes.remove(shoe_number)
            money_earned += money_purchased
            break
print(money_earned)
